...
return the array with each digit incremented by its position in the array: the first digit will be incremented by 1, the second digit by 2, etc. 
...
def last_digit(arr):
    return [x % 10 if x > 9 else x for x in arr]
